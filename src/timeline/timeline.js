...
export class TimeElement extends React.Component {
	...
	getPosition(start_ms, end_ms) {
		...
		const twidth_ms =
			(this.context.endYear - this.context.startYear + 2) * year_ms;

		const pstart_ms = Math.max(
			start_ms,
			Date.parse(this.context.startYear) - year_ms * 0.5,
		);
		const pend_ms = Math.min(end_ms, tend_ms);
	...
	render() {
		...
		return (
			<a
				...
				style={{ left: `${left_p}%`, width: `${width_p}%` }}
			>
				{this.props.children}
			</a>
		);
...
export class Event extends TimeElement {
	...
	render() {
		...
		return <div className="event" style={{ left: `${left_p}%` }} />;
	}
}
